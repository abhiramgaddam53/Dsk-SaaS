...
export const spellNumberIndian = (numInput: string | number): string => {
  ...
  const a = ['','One','Two','Three','Four','Five','Six','Seven','Eight','Nine','Ten','Eleven','Twelve','Thirteen','Fourteen','Fifteen','Sixteen','Seventeen','Eighteen','Nineteen'];
  const b = ['','','Twenty','Thirty','Forty','Fifty','Sixty','Seventy','Eighty','Ninety'];
  const getWords = (n: string) => { const num = Number(n); return (num > 19 ? b[Math.floor(num/10)] + (num%10 !== 0 ? ' '+a[num%10] : '') : a[num]).trim(); };
  if (numStr.length > 9) return 'Amount too large';
  const n = ('000000000' + numStr).slice(-9);
  let out = '';
  if (Number(n.slice(0,2))>0) out += getWords(n.slice(0,2))+' Crores ';
  ...
  if (Number(n.slice(6,7))>0) out += getWords(n.slice(6,7))+' Hundred ';
  if (Number(n.slice(7,9))>0) { if (out!=='') out+='and '; out+=getWords(n.slice(7,9)); }
  return (out.trim() === 'One' ? 'One Rupee' : out.trim()+' Rupees');
};

const parseNum = (v: string|number|undefined) => parseFloat((v??'0').toString().replace(/,/g,''))||0;
const fmtINR   = (n: number) => n > 0 ? Math.round(n).toLocaleString('en-IN') : '0';
...
function SubLabel({ label }: { label: string }) {
  return <p className="text-[10px] font-bold text-gray-500 mb-1.5 mt-3 uppercase tracking-wide border-b border-gray-200 pb-0.5">{label}</p>;
}
const InputField = ({ label, name, value, onChange }: any) => (
  <div className="mb-2">
    <label className="block text-[10px] font-semibold text-gray-500 uppercase mb-0.5 leading-tight">{label}</label>
...
// Auto-calculated field — pre-filled but still editable (no readOnly)
const CalcField = ({ label, name, value, onChange }: any) => (
  <div className="mb-2">
    <label className="block text-[10px] font-semibold text-[#00a0ef] uppercase mb-0.5 leading-tight">{label} <span className="text-[9px] normal-case font-normal text-gray-400">(auto)</span></label>
    <input type="text" name={name} value={value??''} onChange={onChange}
      className="w-full px-2 py-1 border border-[#00a0ef]/40 rounded text-[11.5px] text-gray-800 bg-blue-50/30 focus:outline-none focus:border-[#00a0ef] focus:ring-1 focus:ring-[#00a0ef] transition-colors" />
  </div>
);
const RadioGroup = ({ label, name, options, value, onChange }: any) => (
  <div className="mb-3 bg-gray-50 border border-gray-200 rounded p-2">
...
const RadioMoney = ({ label, name, options, value, onChange }: { label:string;name:string;options:number[];value:string;onChange:any }) => (
  <div className="mb-3 bg-gray-50 border border-gray-200 rounded p-2">
    ...
    <div className="flex flex-wrap gap-1.5">
      {options.map(opt => {
        const sv = opt===0?'0':opt.toLocaleString('en-IN');
        return (
          <label key={opt} className={`flex items-center gap-1 text-[11px] cursor-pointer px-2 py-0.5 rounded border transition-colors ${value===sv||value===opt.toString()?'bg-blue-100 border-blue-400 text-blue-800 font-bold':'border-gray-300 text-gray-700 hover:bg-gray-100'}`}>
            <input type="radio" name={name} value={sv} checked={value===sv||value===opt.toString()} onChange={onChange} className="accent-[#00a0ef]" />
            {opt===0?'Nil / 0':`₹${opt.toLocaleString('en-IN')}`}
          </label>
        );
      })}
    ...
    <div className="mt-1.5">
      <input type="text" name={name} value={value??''} onChange={onChange} placeholder="or type custom value..."
        className="w-full px-2 py-0.5 border border-dashed border-gray-300 rounded text-[10.5px] text-gray-700 bg-white focus:outline-none focus:border-[#00a0ef] transition-colors" />
    </div>
  </div>
);
...
const OPT_HIGH_MIDDLE_POOR=["High","Middle","Poor"];
const OPT_URBAN_RURAL=["Urban","Semi Urban","Rural"];
const OPT_LOCALITY_CLASSIFICATION=["Residential Zone as per Master Plan","Mixed residntial cum commercial Zone","Industrial Classification with few residential dwellings nearby","falls under semi-urban/peri-urban category"];
const OPT_SURROUNDING_DEV=["Surrounding area is well developed with paved roads, houses, and shops","Moderately developed area with upcoming residential layouts","Developing area with few built-up properties and open plots","Underdeveloped area with limited civic infrastructure"];
const OPT_FLOODING=["No history of flooding; land level is higher than adjoining road.","Minor waterlogging during heavy rains, otherwise safe.","Low-lying area prone to temporary stagnation during monsoon","Located in flood-prone zone; precaution required during heavy rainfall."];
const OPT_CIVIC=["All major civic amenities are available within 1 km radius.","Schools and shops nearby; hospital and market within moderate distance.","Basic amenities available; higher-order facilities at 2–3 km distance.","Limited civic amenities; accessibility depends on private transport."];
const OPT_LAND_LEVEL=["Land is level and matches surrounding ground.","Gently sloping terrain with proper drainage.","Slightly elevated land providing good drainage.","Low-lying land requiring filling to match road level.","Marsh"];
const OPT_SHAPE=["Regular rectangular plot.","Square-shaped plot suitable for efficient development.","Irregular shape but usable without much constraint.","Triangular/narrow plot with some planning limitations."];
const OPT_TYPE_USE=["Suitable for residential construction.","Suitable for residential-cum-commercial use.","Suitable for industrial/warehouse use.","Suitable for institutional or office building purpose."];
const OPT_USAGE_RESTRICTION=["No known usage restriction as per local authority.","Subject to residential use only.","Use restricted as per layout or development control rules.","Within agricultural zone — conversion required for non-agricultural use."];
const OPT_TOWN_PLANNING=["Yes, within approved layout as per local authority.","Not in approved layout but falls in LRS Regularized Area","Unapproved layout; independent site outside planning area","Within GP Limits Approval","Within GP Limits"];
const OPT_CORNER=["Intermittent plot with single road frontage.","Corner plot with two-side road frontage.","End plot of a road with one open side.","Interior plot with access via internal road."];
const OPT_ROAD_FACILITIES=["Public Road","Private Road","Good road connectivity with public transport access.","Moderate road width and connectivity.","Limited road access with narrow approach."];
const OPT_TYPE_ROAD=["WBM Road","CC Road","BT Road","Kacha Road","Internal Road"];
const OPT_WIDTH_ROAD=["Above 20' Wide","Below 20 Ft Wide"];
const OPT_LAND_LOCKED=["No, plot has direct road access.","Access available through internal layout road.","Indirect access through private pathway / right of way.","Yes land-locked without proper approach."];
const OPT_WATER=["Borewell / open well source available.","Municipal water supply available.","Both municipal and borewell water sources available","No assured water source; to be arranged independently."];
const OPT_SEWERAGE=["Septic tank system in use; no underground network.","Drainage under development by local authority","Underground drainage system available","No formal drainage; open stormwater drains present."];
const OPT_POWER=["Power supply available through local electricity board.","Power poles in vicinity; connection to be obtained.","No power supply network presently available."];
const OPT_ADVANTAGES=["Nil","All amenities are with in walkable distance, it is fully developed residential area","It is in the prime locality with high market demand for Housing development"];
const OPT_TYPE_BUILDING=["Independent residential house (single-family dwelling)","Commercial building / shop / office premises","Apartment / Flat in a multi-storied residential complex","Industrial / warehouse / factory structure","Mixed-use building (residential with commercial portion)"];
const OPT_TYPE_CONSTRUCTION=["RCC framed structure with brick walls and reinforced concrete roof.","Load-bearing structure with brick walls and RCC / slab roofing.","Semi-pucca structure with brick walls and tiled / ACC sheet roof.","Temporary structure with tin / asbestos roofing and light foundations.","Steel / pre-engineered industrial shed with sheet roofing."];
const OPT_QUALITY=["Good quality construction with superior materials and workmanship","Average quality construction using standard materials","Fair quality construction with moderate finishing","Poor quality construction with basic materials and limited finish.","Superior quality with high-end specifications and architectural detailing"];
const OPT_APPEARANCE=["Appears neat, well-designed, and aesthetically pleasing.","Functional and presentable appearance suitable for the locality.","Moderately appealing, showing minor signs of age or wear.","Dull / outdated appearance requiring renovation.","Modern elevation with good architectural finish."];
const OPT_MAINTENANCE=["Well maintained; in good habitable condition","Fairly maintained; minor repairs required","Poorly maintained; shows signs of neglect and deterioration","Recently constructed and in excellent condition","Old structure but maintained satisfactorily."];
const OPT_DRAWING_APPROVAL=["As per copy provided by the applicant, layout approval was issued on [insert date]; validity period not mentioned in the plan.","Approval dated [insert date] valid as per norms of the concerned authority.","Copy of layout approval not furnished; hence validity could not be verified.","Layout plan approval not applicable, as the property falls under old developed area without layout sanction requirement."];
const OPT_MAP_AUTHORITY=["As per copy produced, approval is issued by Municipal Corporation / Town Planning Department / Gram Panchayat / DTCP, etc.","No approval document was provided for verification.","The plan appears to have been sanctioned by the local authority based on markings and signatures seen on the plan copy.","Layout plan submitted is an unapproved sketch and not authenticated by any statutory authority."];
const OPT_GENUINENESS=["Verified based on the stamped/signed copy furnished by the client.","Physical verification of genuineness with the issuing authority has not been carried out — authenticity assumed based on documents provided.","Could not be verified with the authority due to non-availability of online or official verification mechanism","Document appears genuine in format and content, but no independent cross-verification was possible."];
const OPT_VALUER_COMMENTS=["Approval document appears in order and consistent with the property layout observed at site.","The approval plan bears official stamps but genuineness could not be independently confirmed.","Plan copy does not bear clear reference number or sanction details; hence authenticity could not be confirmed.","The property layout and construction appear broadly in line with the plan submitted."];
const OPT_FOUNDATION=["Random rubble masonry foundation in cement mortar below plinth level; standard PCC bed.","RCC column and footing type foundation with adequate reinforcement as per structural design.","RCC isolated/pedestal footing with plinth beams; anti-termite treatment provided.","Pile/raft foundation with advanced waterproofing and soil stabilization measures."];
const OPT_BASEMENT=["Vitrified tiles flooring in all rooms; anti-skid ceramic tiles in toilets and balconies.","Marble flooring in main areas; ceramic tiles in kitchen and toilets.","Polished Kota stone flooring with cement skirting.","No basement provided; only plinth level structure.","Granite flooring in living area, vitrified in bedrooms, and ceramic in service areas."];
const OPT_SUPER_STRUCTURE=["Brick masonry walls in cement mortar (1:6) for superstructure. 9\" Thick for External and 4\" Thick for Internal Walls","Solid concrete block masonry for external and internal walls.","Burnt clay brick masonry with RCC framed structure.","Laterite / red brick masonry plastered with cement mortar both sides."];
const OPT_JOINERY=["Doors: Wooden frames with laminated flush shutters. Windows: Steel casement type with glazing and MS grill protection. Timber: Teak / country wood used for frames.","Teak / Hardwood frames with panelled shutters. Windows: Wooden casement with glass panes.","Steel frames with steel panel shutters. Windows: Steel fixed/sliding with MS grills.","UPVC doors and windows with double-glazed glass panels."];
const OPT_RCC=["RCC works carried out with M20 grade concrete using tor steel reinforcement as per structural design.","RCC works in M15 grade concrete with mild steel reinforcement.","Lintel beams and slab provided; no RCC columns (load-bearing).","Not applicable."];
const OPT_PLASTERING=["Single coat 12mm thick cement plaster in 1:6 (Cement : Sand) proportion for internal and external walls; no finishing coat.","Two coat plastering – 15mm rough coat + 6mm finishing coat with smooth finish.","Textured / rough cast exterior plaster; smooth internal plaster.","Gypsum plaster on internal walls; cement plaster on external surfaces."];
const OPT_FLOORING=["Vitrified tiles flooring in all rooms; anti-skid ceramic tiles in toilets and balconies.","Marble flooring in main areas; ceramic tiles in kitchen and toilets.","Polished Kota stone flooring with cement skirting.","Cement concrete flooring finished with red oxide / IPS flooring.","Granite flooring in living area, vitrified in bedrooms, and ceramic in service areas."];
const OPT_SPECIAL_FINISH=["Nil","Wooden Panelling works in interiors","Marble Carving works","Granite finishes in the kitchen","Granite flooring in living area, vitrified in bedrooms, and ceramic in service areas."];
const OPT_ROOFING=["Reinforced Cement Concrete (RCC) flat roof with weathering course.","RCC slab roof finished with water-proofing treatment.","Sloped roof with Mangalore tile covering on steel / RCC truss.","RCC slab roof covered with terrace tiles."];
const OPT_DRAINAGE=["Open surface drains along plot periphery; connected to municipal/roadside drain.","Partly covered cement drains with proper slope; connected to septic tank or municipal line.","Underground PVC drainage system with inspection chambers and gully traps.","RCC underground drainage network with manholes, inspection chambers, and connection to city sewerage system"];
const OPT_WIRING=["Concealed PVC wiring in rigid conduit","Surface wiring in casing capping","Open wiring with wooden battens"];
const OPT_FITTINGS=["Superior – Modular switches (Anchor, Havells, GM, etc.) with concealed boxes","Ordinary – Piano type switches and basic ceiling fittings","Poor – Mixed or outdated fittings, temporary wiring, exposed joints"];
const OPT_LIGHT_POINTS=["Moderate – 1 per 80–100 sq.ft.","Sufficient – 1 per 60–80 sq.ft.","Extensive – decorative lighting / false ceiling LED system"];
const OPT_FAN_POINTS=["One per habitable room","One per room + additional in living/dining","Ceiling fans with regulators in each major room"];
...
const MAP_PHOTO_LABELS = [
  ...
  "Roadmap – Wide Angle (area context + polygon)",
  "Satellite – Close-up View (site highlighted)",
  "Satellite – Wide Angle (area context + polygon)",
];

// ── Main Sidebar ──────────────────────────────────────────────────────────────
export default function BuildingValuationSidebar({
...
console.log("SideBar DAta :",formData)
  ...
  const handleInterceptChange = (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    // Auto-words
    const wordsMap: Record<string,string> = { fairMarketValue:'fairMarketValueWords', bookValue:'bookValueWords', distressValue:'distressValueWords', realisableValue:'realisableValueWords' };
    if (wordsMap[name]) fire(wordsMap[name], spellNumberIndian(value));
    ...
    if (name==='yearOfConstruction') {
      ...
      if (!isNaN(yr)) {
        ...
        fire('ageOfBuilding', String(age));
        fire('groundFloorAge', String(age));
        fire('totalLifeOfBuilding', '80');
        fire('expectedFutureLife', String(Math.max(0,80-age)));
      }
    }
    ...
    if (name==='groundFloorReplacementCost'||name==='groundFloorDepreciation') {
      ...
      const dep  = parseNum(name==='groundFloorDepreciation'?value:formData.groundFloorDepreciation);
      const net  = cost-dep;
      fire('groundFloorNetValue', fmtINR(net));
      fire('buildingTotal',  fmtINR(net));
      fire('buildingPMR',    fmtINR(net));
      recalcAbstract(name, value, net);
    }
    ...
    if (name==='extentForValuation'||name==='guidelineRate') {
      const ext=parseNum(name==='extentForValuation'?value:formData.extentForValuation);
      const glr=parseNum(name==='guidelineRate'?value:formData.guidelineRate);
      const lGLR=fmtINR(ext*glr);
      fire('landValueGLR',lGLR); fire('landGLR',lGLR);
    }
    // Land PMR
    if (name==='unitRatePMR'||name==='extentForValuation') {
      const ext=parseNum(name==='extentForValuation'?value:formData.extentForValuation);
      const pmr=parseNum(name==='unitRatePMR'?value:formData.unitRatePMR);
      const lPMR=fmtINR(ext*pmr);
      fire('landValuePMR',lPMR); fire('landPMR',lPMR);
    }

    // Part C
    const partC=['portico','ornamentalDoor','sitOutVerandah','overheadWaterTank','extraGates'];
    if (partC.includes(name)) {
      const snap={...formData,[name]:value};
      const t=partC.reduce((s,f)=>s+parseNum(snap[f as keyof FormData]),0);
      fire('extraItemsTotal',fmtINR(t)); fire('extraItemsAbstract',fmtINR(t));
    }

    // Part D
    const partD=['wardrobes','glazedTiles','extraSinksBathtub','marbleCeramicTiles','interiorDecorations','architecturalElevation','panellingWorks','aluminiumWorks','aluminiumHandRails','falseCeilingValue'];
    if (partD.includes(name)||name==='falseCeilingArea'||name==='falseCeilingRate') {
      const area=parseNum(name==='falseCeilingArea'?value:formData.falseCeilingArea);
      const rate=parseNum(name==='falseCeilingRate'?value:formData.falseCeilingRate);
      const fcv=area*rate;
      if (name==='falseCeilingArea'||name==='falseCeilingRate') fire('falseCeilingValue',fmtINR(fcv));
      const snap={...formData,[name]:value,falseCeilingValue:name==='falseCeilingArea'||name==='falseCeilingRate'?fmtINR(fcv):formData.falseCeilingValue};
      const t=partD.reduce((s,f)=>s+parseNum(snap[f as keyof FormData]),0);
      fire('amenitiesTotal',fmtINR(t)); fire('amenitiesAbstract',fmtINR(t));
    }

    // Part E Misc
    const partEM=['separateLumberRoom','separateToiletRoom','separateWaterTank','treesGardening'];
    if (partEM.includes(name)) {
      const snap={...formData,[name]:value};
      const t=partEM.reduce((s,f)=>s+parseNum(snap[f as keyof FormData]),0);
      fire('miscTotal',fmtINR(t)); fire('miscAbstract',fmtINR(t));
    }

    // Part E Services
    const svcKeys=['openWell','deepBore','handPump','motor','corporationTap','underGroundSump','overheadTankServices','septicTank','undergroundSewage','compoundWallValue','pavementsValue'];
    if (svcKeys.includes(name)||name==='compoundWallRft'||name==='compoundWallRate'||name==='pavementsSqft'||name==='pavementsRate') {
      const cwRft=parseNum(name==='compoundWallRft'?value:formData.compoundWallRft);
      const cwRate=parseNum(name==='compoundWallRate'?value:formData.compoundWallRate);
      const cwVal=cwRft*cwRate;
      if (name==='compoundWallRft'||name==='compoundWallRate') fire('compoundWallValue',fmtINR(cwVal));
      ...
      const snap={...formData,[name]:value,
        compoundWallValue:name==='compoundWallRft'||name==='compoundWallRate'?fmtINR(cwVal):formData.compoundWallValue,
        pavementsValue:name==='pavementsSqft'||name==='pavementsRate'?fmtINR(pvVal):formData.pavementsValue};
      const t=svcKeys.reduce((s,f)=>s+parseNum(snap[f as keyof FormData]),0);
      fire('servicesTotal',fmtINR(t)); fire('servicesAbstract',fmtINR(t));
    }
  };

  const recalcAbstract = (name: string, value: string, netBuilding?: number) => {
    const snap={...formData,[name]:value};
    const landGLR=parseNum(snap.landGLR), landPMR=parseNum(snap.landPMR);
    const bldGLR=parseNum(snap.buildingGLR), bldPMR=netBuilding??parseNum(snap.buildingPMR);
    const extraC=parseNum(snap.extraItemsAbstract), amenD=parseNum(snap.amenitiesAbstract);
    const miscE=parseNum(snap.miscAbstract), svcF=parseNum(snap.servicesAbstract);
    const totalGLR=landGLR+bldGLR, totalPMR=landPMR+bldPMR+extraC+amenD+miscE+svcF;
    fire('totalGLR',fmtINR(totalGLR)); fire('totalPMR',fmtINR(totalPMR));
    const pmv=Math.round(totalPMR/1000)*1000;
    fire('presentMarketValue',fmtINR(pmv));
    fire('fairMarketValue',fmtINR(pmv)); fire('fairMarketValueWords',spellNumberIndian(pmv));
    fire('distressValue',fmtINR(Math.round(pmv*0.80/100)*100)); fire('distressValueWords',spellNumberIndian(Math.round(pmv*0.80/100)*100));
    fire('realisableValue',fmtINR(Math.round(pmv*0.90/100)*100)); fire('realisableValueWords',spellNumberIndian(Math.round(pmv*0.90/100)*100));
    fire('bookValue',fmtINR(Math.round(totalGLR/1000)*1000)); fire('bookValueWords',spellNumberIndian(Math.round(totalGLR/1000)*1000));
  };
  ...
  const generateMapPhotos = () => {
    ...
    const apiKey =  process.env.NEXT_PUBLIC_GOOGLE_MAPS_STATIC_API_KEY ; 
    const points: string[] = [];

    // 1. Safely extract points
    for (let i = 1; i <= 10; i++) {
      // If it's the first point and empty, fallback to the primary coordinates
      let lat = (formData as any)[`mapLat${i}`];
      let lng = (formData as any)[`mapLng${i}`];

      if (i === 1 && !lat) lat = formData.latitude;
      if (i === 1 && !lng) lng = formData.longitude;

      // Clean the string and ensure it's a valid number
    ...
    if (points.length === 0) {
      alert("Please enter at least one valid coordinate (Latitude/Longitude).");
      return;
    }

    // 2. Safe Center Calculation (prevents NaN errors)
    let centerParam = "";
    if (points.length === 1) {
      centerParam = points[0];
    } else {
      const lats = points.map(p => parseFloat(p.split(',')[0]));
      const lngs = points.map(p => parseFloat(p.split(',')[1]));
      centerParam = `${(Math.min(...lats) + Math.max(...lats)) / 2},${(Math.min(...lngs) + Math.max(...lngs)) / 2}`;
    }

    // 3. Markers vs Path (CRITICAL FOR GOOGLE MAPS)
    // Google API crashes if you use "path=" with only 1 point. We must use "markers=" instead.
    let overlayParam = "";
    if (points.length === 1) {
      overlayParam = `markers=color:red|${points[0]}`;
    } else if (points.length === 2) {
      overlayParam = `path=color:0xff0000ff|weight:3|${points.join('|')}`;
    } else {
      // 3+ points creates a filled red polygon
      overlayParam = `path=color:0xff0000ff|weight:2|fillcolor:0xff000033|${points.join('|')}`;
    }

    // 4. Build URLs
    // Close-up: Omitting center/zoom forces Google to perfectly auto-crop to the marker/polygon
    const urlRoadmapClose = `https://maps.googleapis.com/maps/api/staticmap?size=600x400&maptype=roadmap&${overlayParam}&zoom=21&key=${apiKey}`;
    const urlSatelliteClose = `https://maps.googleapis.com/maps/api/staticmap?size=600x400&maptype=satellite&${overlayParam}&zoom=21&key=${apiKey}`;

    // Wide-angle: We force a specific center and zoom=15 to show the surrounding neighborhood
    const urlRoadmapWide = `https://maps.googleapis.com/maps/api/staticmap?size=600x400&maptype=roadmap&center=${centerParam}&zoom=18&${overlayParam}&key=${apiKey}`;
    const urlSatelliteWide = `https://maps.googleapis.com/maps/api/staticmap?size=600x400&maptype=satellite&center=${centerParam}&zoom=18&${overlayParam}&key=${apiKey}`;

    const generatedPhotos = [
      { url: urlRoadmapClose, label: "Roadmap – Close-up View (site highlighted)" },
      { url: urlRoadmapWide, label: "Roadmap – Wide Angle (area context + polygon)" },
      { url: urlSatelliteClose, label: "Satellite – Close-up View (site highlighted)" },
      { url: urlSatelliteWide, label: "Satellite – Wide Angle (area context + polygon)" },
    ];

    onMapPhotosChange(generatedPhotos);
  };

  // Map photo handler for each slot
  const handleMapFile = (idx: number, files: FileList|null) => {
    if (!files||!files[0]) return;
    const next = [...mapPhotos];
    if (next[idx]) URL.revokeObjectURL(next[idx].url);
    next[idx] = { url:URL.createObjectURL(files[0]), label:MAP_PHOTO_LABELS[idx] };
    onMapPhotosChange(next);
  };
  const removeMapPhoto = (idx: number) => {
    const next = [...mapPhotos];
    if (next[idx]) URL.revokeObjectURL(next[idx].url);
    next[idx] = { url:'', label:MAP_PHOTO_LABELS[idx] };
    onMapPhotosChange(next);
  };
  ...
  return (
    ...
    <fieldset disabled={locked} style={{ display: 'contents' }}>
      ...
      <div className="mb-6 border border-blue-100 bg-blue-50/30 p-3 rounded-lg">
        <SectionHeader label="5 · Valuation Summary" />
        <p className="text-[9.5px] text-blue-600 mb-2 italic pl-1">Auto-calculated from Part F. Edit freely to apply discounts or adjustments.</p>
        <div className="pl-1 grid grid-cols-2 gap-2">
          <CalcField label="Fair Market Value (Rs)" name="fairMarketValue" value={formData.fairMarketValue} onChange={handleInterceptChange} />
          <div className="col-span-2"><TextArea label="In Words" name="fairMarketValueWords" value={formData.fairMarketValueWords} onChange={handleInterceptChange} rows={1} /></div>
          <CalcField label="Book / Govt Value (Rs)" name="bookValue" value={formData.bookValue} onChange={handleInterceptChange} />
          <div className="col-span-2"><TextArea label="In Words" name="bookValueWords" value={formData.bookValueWords} onChange={handleInterceptChange} rows={1} /></div>
          <CalcField label="Distress Value (Rs)" name="distressValue" value={formData.distressValue} onChange={handleInterceptChange} />
          <div className="col-span-2"><TextArea label="In Words" name="distressValueWords" value={formData.distressValueWords} onChange={handleInterceptChange} rows={1} /></div>
          <CalcField label="Realisable Value (Rs)" name="realisableValue" value={formData.realisableValue} onChange={handleInterceptChange} />
          <div className="col-span-2"><TextArea label="In Words" name="realisableValueWords" value={formData.realisableValueWords} onChange={handleInterceptChange} rows={1} /></div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <SectionHeader label="7 · Documents Produced" />
        <div className="pl-3">
          {[["Document 1","doc1Description","doc1No","doc1Date","doc1Copy"],["Document 2","doc2Description","doc2No","doc2Date","doc2Copy"],["Document 3","doc3Description","doc3No","doc3Date","doc3Copy"]].map(([sub,desc,no,dt,copy])=>(
            <div key={sub}>
              <SubLabel label={sub} />
              <div className="grid grid-cols-2 gap-2">
                <InputField label="Description" name={desc} value={(formData as any)[desc]} onChange={handleInterceptChange} />
      ...
      {/* ══ 13. SITE EXTENT ══ */}
      <div className="mb-6">
        <SectionHeader label="13 · Site Extent" />
        <div className="grid grid-cols-2 gap-2 pl-3">
          <InputField label="Extent (Sq.Yds)" name="extentSqYds" value={formData.extentSqYds} onChange={handleInterceptChange} />
          <InputField label="Rate (Rs/Sq.Yd)" name="landRatePerSqYd" value={formData.landRatePerSqYd} onChange={handleInterceptChange} />
          <InputField label="Extent as per Deed" name="extentAsPerDeed" value={formData.extentAsPerDeed} onChange={handleInterceptChange} />
      ...
      <div className="mb-6">
        ...
        <div className="grid grid-cols-2 gap-2 pl-3">
          <InputField label="Guideline Rate (Rs/Sq.Yd)" name="guidelineRate" value={formData.guidelineRate} onChange={handleInterceptChange} />
          <CalcField label="Land Value by GLR (Rs)" name="landValueGLR" value={formData.landValueGLR} onChange={handleInterceptChange} />
          <InputField label="PMR Range" name="prevailingMarketRateRange" value={formData.prevailingMarketRateRange} onChange={handleInterceptChange} />
          <InputField label="Unit Rate PMR (Rs/Sq.Yd)" name="unitRatePMR" value={formData.unitRatePMR} onChange={handleInterceptChange} />
          <CalcField label="Land Value by PMR (Rs)" name="landValuePMR" value={formData.landValuePMR} onChange={handleInterceptChange} />
          <InputField label="Compound Wall (extra)" name="CompoundWall" value={(formData as any).CompoundWall} onChange={handleInterceptChange} />
          <InputField label="Gate (extra)" name="Gate" value={(formData as any).Gate} onChange={handleInterceptChange} />
          <InputField label="Power (extra)" name="power" value={(formData as any).power} onChange={handleInterceptChange} />
        </div>
      </div>
      ...
      <div className="mb-6">
        <SectionHeader label="15 · Building Technical Details – Part B" />
        <p className="text-[9.5px] text-blue-600 mb-2 pl-3 italic">Age, Total Life & Future Life auto-calculate from Year of Construction — edit freely.</p>
        <div className="pl-3">
          <RadioGroup label="Type of Building" name="typeOfBuilding" options={OPT_TYPE_BUILDING} value={formData.typeOfBuilding} onChange={handleInterceptChange} />
          <RadioGroup label="Type of Construction" name="typeOfConstruction" options={OPT_TYPE_CONSTRUCTION} value={formData.typeOfConstruction} onChange={handleInterceptChange} />
          <RadioGroup label="Quality of Construction" name="qualityOfConstruction" options={OPT_QUALITY} value={formData.qualityOfConstruction} onChange={handleInterceptChange} />
          ...
          <div className="grid grid-cols-2 gap-2">
            <InputField label="Year of Construction" name="yearOfConstruction" value={formData.yearOfConstruction} onChange={handleInterceptChange} />
            <CalcField label="Age of Building" name="ageOfBuilding" value={formData.ageOfBuilding} onChange={handleInterceptChange} />
            <CalcField label="Total Life (yrs)" name="totalLifeOfBuilding" value={formData.totalLifeOfBuilding} onChange={handleInterceptChange} />
            <CalcField label="Expected Future Life" name="expectedFutureLife" value={formData.expectedFutureLife} onChange={handleInterceptChange} />
          </div>
          <TextArea label="No. of Floors & Height" name="numberOfFloorsHeight" value={formData.numberOfFloorsHeight} onChange={handleInterceptChange} rows={1} />
        </div>
      </div>
      ...
      <div className="mb-6 border border-green-100 bg-green-50/30 p-3 rounded-lg">
        <SectionHeader label="21 · Building Valuation Details" />
        <p className="text-[9.5px] text-green-600 mb-2 italic">Net Value = Replacement Cost − Depreciation (auto-calculated, editable).</p>
        <div className="pl-1">
          <SubLabel label="Ground Floor" />
          <div className="grid grid-cols-2 gap-2">
            <InputField label="Plinth Area (Sq.ft)" name="groundFloorPlinthArea" value={formData.groundFloorPlinthArea} onChange={handleInterceptChange} />
            <InputField label="Rate (Rs/Sq.ft)" name="groundFloorRate" value={formData.groundFloorRate} onChange={handleInterceptChange} />
            <CalcField label="Age of Building" name="groundFloorAge" value={formData.groundFloorAge} onChange={handleInterceptChange} />
            <InputField label="Replacement Cost (Rs)" name="groundFloorReplacementCost" value={formData.groundFloorReplacementCost} onChange={handleInterceptChange} />
            <InputField label="Depreciation (Rs)" name="groundFloorDepreciation" value={formData.groundFloorDepreciation} onChange={handleInterceptChange} />
            <CalcField label="Net Value after Dep. (Rs)" name="groundFloorNetValue" value={formData.groundFloorNetValue} onChange={handleInterceptChange} />
          </div>
          <CalcField label="Building Total (Rs)" name="buildingTotal" value={formData.buildingTotal} onChange={handleInterceptChange} />
        </div>
      </div>
      ...
      <div className="mb-6">
        <SectionHeader label="22 · Extra Items – Part C (Rs)" />
        <p className="text-[9.5px] text-gray-500 mb-2 pl-3 italic">Select preset value or type custom — Total auto-sums.</p>
        <div className="pl-3">
          <RadioMoney label="Portico" name="portico" options={[0,20000,35000,55000,85000]} value={formData.portico} onChange={handleInterceptChange} />
          <RadioMoney label="Ornamental Front / Pooja Door" name="ornamentalDoor" options={[0,10000,20000,35000,60000]} value={formData.ornamentalDoor} onChange={handleInterceptChange} />
          <RadioMoney label="Sit Out / Verandah with Steel Grills" name="sitOutVerandah" options={[0,8000,15000,25000,45000]} value={formData.sitOutVerandah} onChange={handleInterceptChange} />
          ...
          <RadioMoney label="Extra Steel / Collapsible Gates" name="extraGates" options={[0,12000,20000,35000,55000]} value={formData.extraGates} onChange={handleInterceptChange} />
          <div className="bg-orange-50 border border-orange-200 rounded p-2 mt-1">
            <CalcField label="Extra Items Total (Rs)" name="extraItemsTotal" value={formData.extraItemsTotal} onChange={handleInterceptChange} />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <SectionHeader label="23 · Amenities – Part D (Rs)" />
        <p className="text-[9.5px] text-gray-500 mb-2 pl-3 italic">Select preset or type custom — Total auto-sums.</p>
        <div className="pl-3">
          <RadioMoney label="Wardrobes / Show Cases / Wooden Cubboards" name="wardrobes" options={[0,20000,35000,65000,100000]} value={formData.wardrobes} onChange={handleInterceptChange} />
          <RadioMoney label="Glazed Tiles" name="glazedTiles" options={[0,10000,15000,25000,40000]} value={formData.glazedTiles} onChange={handleInterceptChange} />
          <RadioMoney label="Extra Sinks and Bath Tub" name="extraSinksBathtub" options={[0,5000,12000,25000,50000]} value={formData.extraSinksBathtub} onChange={handleInterceptChange} />
          ...
          <div className="grid grid-cols-2 gap-2">
            <InputField label="Area (Sq.ft)" name="falseCeilingArea" value={formData.falseCeilingArea} onChange={handleInterceptChange} />
            <InputField label="Rate (Rs/Sq.ft)" name="falseCeilingRate" value={formData.falseCeilingRate} onChange={handleInterceptChange} />
            <CalcField label="False Ceiling Value (Rs)" name="falseCeilingValue" value={formData.falseCeilingValue} onChange={handleInterceptChange} />
          </div>
          <div className="bg-orange-50 border border-orange-200 rounded p-2 mt-1">
            <CalcField label="Amenities Total (Rs)" name="amenitiesTotal" value={formData.amenitiesTotal} onChange={handleInterceptChange} />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        ...
        <div className="pl-3">
          ...
          <RadioMoney label="Trees, Gardening" name="treesGardening" options={[0,10000,25000,50000,100000]} value={formData.treesGardening} onChange={handleInterceptChange} />
          <div className="bg-orange-50 border border-orange-200 rounded p-2 mt-1">
            <CalcField label="Miscellaneous Total (Rs)" name="miscTotal" value={formData.miscTotal} onChange={handleInterceptChange} />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6">
        <SectionHeader label="25 · Services – Part E (Rs)" />
        <p className="text-[9.5px] text-gray-500 mb-2 pl-3 italic">Select preset or type custom — Compound Wall & Pavements auto-multiply — Total auto-sums.</p>
        <div className="pl-3">
          <SubLabel label="Water Supply Arrangements" />
          <RadioMoney label="Open Well" name="openWell" options={[0,25000,40000,65000,90000]} value={formData.openWell} onChange={handleInterceptChange} />
          <RadioMoney label="Deep Bore" name="deepBore" options={[0,45000,65000,90000,120000]} value={formData.deepBore} onChange={handleInterceptChange} />
          ...
          <RadioMoney label="Underground Sewerage" name="undergroundSewage" options={[0,10000,18000,30000,50000]} value={formData.undergroundSewage} onChange={handleInterceptChange} />
          <SubLabel label="Compound Wall (Services Section)" />
          <p className="text-[9.5px] text-gray-400 mb-1">Value = Length × Rate (auto-calculated, editable)</p>
          <div className="grid grid-cols-2 gap-2">
            <InputField label="Length (R.ft)" name="compoundWallRft" value={formData.compoundWallRft} onChange={handleInterceptChange} />
            <InputField label="Rate (Rs/rft)" name="compoundWallRate" value={formData.compoundWallRate} onChange={handleInterceptChange} />
            <InputField label="Height" name="compoundWallHeight" value={formData.compoundWallHeight} onChange={handleInterceptChange} />
            <InputField label="Length (ft)" name="compoundWallLength" value={formData.compoundWallLength} onChange={handleInterceptChange} />
            <InputField label="Type" name="compoundWallType" value={formData.compoundWallType} onChange={handleInterceptChange} />
            <CalcField label="C.Wall Value (Rs)" name="compoundWallValue" value={formData.compoundWallValue} onChange={handleInterceptChange} />
          </div>
          <SubLabel label="Pavements" />
          <p className="text-[9.5px] text-gray-400 mb-1">Value = Area × Rate (auto-calculated, editable)</p>
          <div className="grid grid-cols-2 gap-2">
            <InputField label="Area (Sq.ft)" name="pavementsSqft" value={formData.pavementsSqft} onChange={handleInterceptChange} />
            <InputField label="Rate (Rs/Sq.ft)" name="pavementsRate" value={formData.pavementsRate} onChange={handleInterceptChange} />
            <CalcField label="Pavements Value (Rs)" name="pavementsValue" value={formData.pavementsValue} onChange={handleInterceptChange} />
          </div>
          <div className="bg-orange-50 border border-orange-200 rounded p-2 mt-2">
            <CalcField label="Services Total (Rs)" name="servicesTotal" value={formData.servicesTotal} onChange={handleInterceptChange} />
          </div>
        </div>
      </div>
      ...
      <div className="mb-6 border border-purple-100 bg-purple-50/20 p-3 rounded-lg">
        <SectionHeader label="26 · Abstract Value – Part F (Rs)" />
        <p className="text-[9.5px] text-purple-600 mb-2 italic">All auto-calculated. Edit any value to apply adjustments.</p>
        <div className="grid grid-cols-2 gap-2 pl-1">
          <CalcField label="Land GLR" name="landGLR" value={formData.landGLR} onChange={handleInterceptChange} />
          <CalcField label="Land PMR" name="landPMR" value={formData.landPMR} onChange={handleInterceptChange} />
          <InputField label="Building GLR" name="buildingGLR" value={formData.buildingGLR} onChange={handleInterceptChange} />
          <CalcField label="Building PMR" name="buildingPMR" value={formData.buildingPMR} onChange={handleInterceptChange} />
          <CalcField label="Extra Items (C)" name="extraItemsAbstract" value={formData.extraItemsAbstract} onChange={handleInterceptChange} />
          <CalcField label="Amenities (D)" name="amenitiesAbstract" value={formData.amenitiesAbstract} onChange={handleInterceptChange} />
          <CalcField label="Miscellaneous (E)" name="miscAbstract" value={formData.miscAbstract} onChange={handleInterceptChange} />
          <CalcField label="Services (F)" name="servicesAbstract" value={formData.servicesAbstract} onChange={handleInterceptChange} />
          <CalcField label="Total GLR" name="totalGLR" value={formData.totalGLR} onChange={handleInterceptChange} />
          <CalcField label="Total PMR" name="totalPMR" value={formData.totalPMR} onChange={handleInterceptChange} />
          <div className="col-span-2">
            <CalcField label="Present Market Value (Rs)" name="presentMarketValue" value={formData.presentMarketValue} onChange={handleInterceptChange} />
          </div>
        </div>
      </div>
      ...
      {/* ══ 28. SITE PHOTOS UPLOAD ══ */}
      <div className="mb-6">
        <SectionHeader label="28 · Site Photos" />
        <p className="text-[9.5px] text-gray-500 mb-3 pl-3">Upload site photos here — they appear in the document pages automatically.</p>
        <div className="pl-3">
          {/* Upload zone */}
          <div
            className="border-2 border-dashed border-blue-300 rounded-lg bg-blue-50 p-4 text-center cursor-pointer hover:bg-blue-100 transition-colors mb-3"
            onClick={() => siteFileRef.current?.click()}
            onDragOver={e=>e.preventDefault()}
            onDrop={e=>{e.preventDefault();handleSiteFiles(e.dataTransfer.files);}}
          >
            ...
            <p className="text-[11px] font-semibold text-blue-600">Click or drag photos here</p>
            <p className="text-[9.5px] text-gray-400 mt-0.5">Any number of photos · JPG, PNG, WEBP · 4 per page</p>
            <input ref={siteFileRef} type="file" accept="image/*" multiple className="hidden" onChange={e=>handleSiteFiles(e.target.files)} />
          </div>
          {/* Thumbnails */}
          {sitePhotos.length > 0 && (
            <div className="grid grid-cols-3 gap-1.5">
              {sitePhotos.map((ph, i) => (
                <div key={i} className="relative border border-gray-200 rounded overflow-hidden bg-gray-50">
                  <img src={ph.url} alt={ph.name} className="w-full h-16 object-contain bg-gray-50" />
                  <button onClick={()=>removeSitePhoto(i)}
                    className="absolute top-0.5 right-0.5 bg-red-500 text-white rounded-full w-4 h-4 flex items-center justify-center text-[9px] hover:bg-red-600 leading-none">×</button>
                  <p className="text-[8px] text-gray-400 truncate px-0.5 pb-0.5 text-center">{i+1}. {ph.name}</p>
                </div>
              ))}
            </div>
          )}
          ...
          {sitePhotos.length > 0 && (
            <>
              <SubLabel label="Photo Captions (optional)" />
              <div className="grid grid-cols-2 gap-1.5">
                {sitePhotos.map((_,i)=>(
                  <InputField key={i} label={`Photo ${i+1}`} name={`photoCaption${i+1}`} value={(formData as any)[`photoCaption${i+1}`]??''} onChange={handleInterceptChange} />
                ))}
              </div>
            </>
          )}
      ...
      {/* ══ 29. MAP PHOTOS & GPS COORDINATES ══ */}
      <div className="mb-6">
        <SectionHeader label="29 · Key Plan Map Photos & GPS Points" />
        <p className="text-[9.5px] text-gray-500 mb-3 pl-3">
          Enter the GPS points below. The system will automatically fetch Roadmap and Satellite views from Google Maps and draw a red polygon over your site.
        </p>

        <div className="pl-3">
          {/* Primary lat/lng */}
          <SubLabel label="Primary Coordinates (Shown in document header)" />
          <div className="grid grid-cols-2 gap-2 mb-4">
            <InputField label="Primary Latitude" name="latitude" value={formData.latitude} onChange={handleInterceptChange} />
            <InputField label="Primary Longitude" name="longitude" value={formData.longitude} onChange={handleInterceptChange} />
          </div>

          {/* GPS Survey Points */}
          <SubLabel label="GPS Polygon Points (Up to 10)" />
          <p className="text-[9.5px] text-gray-500 mb-2">Enter at least 3 points to form a closed polygon shape.</p>
          <div className="grid grid-cols-2 gap-x-2 border border-gray-200 bg-gray-50 p-2 rounded mb-4 max-h-[300px] overflow-y-auto scrollbar-thin">
            {Array.from({length:10}).map((_,i)=>(
              <React.Fragment key={i}>
                <div className="col-span-2">
                  <p className="text-[9px] font-bold text-gray-400 uppercase mt-1 mb-0.5 border-b border-gray-200 pb-0.5">Point {i+1}</p>
                </div>
                <InputField
                  label={`Lat ${i+1}`}
                  name={`mapLat${i+1}`}
          ...
          {/* Generate Button */}
          <button 
            onClick={generateMapPhotos}
            className="w-full bg-[#00a0ef] hover:bg-blue-600 text-white font-bold py-2 rounded shadow transition-colors mb-4 flex items-center justify-center gap-2"
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M17.657 16.657L13.414 20.9a1.998 1.998 0 01-2.827 0l-4.242-4.243a8 8 0 1111.314 0z" />
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
            </svg>
            Fetch 4 Maps via Google API
          </button>

          {/* Generated Maps Preview */}
          {mapPhotos && mapPhotos.length > 0 && (
            <div className="space-y-3">
              <SubLabel label="Generated Maps Preview" />
              {mapPhotos.map((photo, idx) => {
                const mapType = idx < 2 ? '🗺' : '🛰';
                return (
                  <div key={idx} className={`border rounded-lg p-2 ${idx<2?'border-blue-200 bg-blue-50/30':'border-orange-200 bg-orange-50/30'}`}>
                    <p className="text-[10px] font-bold text-gray-700 mb-1">{mapType} Map {idx+1}</p>
                    <p className="text-[9px] text-gray-500 mb-2">{photo.label}</p>
                    {photo.url ? (
                      <div className="relative">
                        {/* Using standard img tag to display the Google Static Map */}
                        <img src={photo.url} alt={photo.label} className="w-full h-auto min-h-[120px] object-cover rounded border border-gray-300 bg-white" />
                      </div>
                    ) : (
                      <div className="h-20 flex items-center justify-center border border-dashed border-gray-300 rounded bg-white">
                        <span className="text-[9px] text-gray-400">Failed to load</span>
                      </div>
                    )}
                  </div>
                );
