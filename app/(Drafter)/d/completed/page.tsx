...
import { Search, ChevronDown, FileText, User, AlertTriangle, ChevronLeft, ChevronRight, Loader, CheckCircle } from 'lucide-react';
import { api } from '@/app/lib/userApis';
import Link from 'next/link';

export default function ReviewedDocumentsPage() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        // 🟢 FILTER: Show only Approved (Completed) or Revision Requested
        const reviewedRecords = formattedData.filter((doc: any) => 
          doc.status === 'approved' || doc.status === 'revision_requested'
        );

        setData(reviewedRecords);
      } catch (error) {
  ...
  // 🟢 DYNAMIC BADGES: Green for Approved, Red for Revision
  ...
  return (
    <div className="max-w-[1400px] mx-auto flex flex-col h-full">
      ...
      <div className="py-1.5 px-4 border-y border-gray-200 ">
        <h1 className="text-xl font-semibold text-gray-900 mb-1">Reviewed Documents</h1>
        <p className="text-sm text-gray-500">View your completed reports or rewrite documents sent back by the Validator.</p>
      </div>
              ...
              safeData.map((doc, index) => (
                <tr key={`${doc.id}-${index}`} className="hover:bg-gray-50 transition-colors">
                  ...
                  <td className="py-3 px-4 whitespace-nowrap">
                    <span className={`flex items-center gap-1.5 w-fit px-2.5 py-1 rounded-full text-[11px] font-semibold tracking-wide border ${getStatusBadge(doc.status)}`}>
                      {doc.status === 'approved' && <CheckCircle className="w-3.5 h-3.5" />}
                      {doc.status === 'revision_requested' && <AlertTriangle className="w-3.5 h-3.5" />}
                      {formatStatus(doc.status)}
                    </span>
                  </td>
                  <td className="py-3 px-4 whitespace-nowrap">
                    {/* 🟢 DYNAMIC BUTTON: "Rewrite" if revision requested, "View" if approved */}
                    <Link 
                      href={`/d/reports/${doc.id}`} 
                      className={`inline-flex px-4 py-1.5 rounded-lg text-sm font-medium transition-colors shadow-sm ${
                        doc.status === 'revision_requested' 
                          ? 'bg-red-600 hover:bg-red-700 text-white' 
                          : 'bg-white border border-gray-300 text-gray-700 hover:bg-gray-50'
                      }`}
                    >
                      {doc.status === 'revision_requested' ? 'Rewrite' : 'View'}
                    </Link>
                  </td>
                </tr>
              ))
      ...
      <div className="flex items-center justify-between mb-4 px-4 pt-2">
        <p className="text-sm text-gray-500">
          Showing <span className="font-semibold text-gray-900">{safeData.length}</span> documents
        </p>
