...
import { registerUser, signInWithGoogle } from "@/app/lib/firebase/authUtils";
...
export default function RegisterPage() {
  ...
  const handleEmailRegister = async (e: React.FormEvent) => {
    e.preventDefault();
    try {
      const { role } = await registerUser(email, password, name, 'site-visitor');
  ...
  const redirectBasedOnRole = (role: string) => {
    if (role === 'Super-admin') router.push('/sa');
    else if (role === 'valuator') router.push('/sa/gen');
    else if (role === 'drafter') router.push('/d');
    else router.push('/s');
  };
  ...
  return (
    <div className="flex min-h-screen items-center justify-center bg-zinc-50">
      <div className="w-full max-w-md space-y-8 rounded-xl bg-white p-10 shadow-md">
        <h2 className="text-center text-3xl font-extrabold text-zinc-900">Create Account</h2>
