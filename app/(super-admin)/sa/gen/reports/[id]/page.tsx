...
export default function DocumentEditorPage() {
  ...
  // Track preview container width for auto-scaling
  const previewContainerRef = useRef<HTMLDivElement>(null);
  const [containerWidth, setContainerWidth] = useState<number>(DOCUMENT_WIDTH);
  const [sitePhotos, setSitePhotos] = useState<{ url: string; name: string }[]>([]);
  const [mapPhotos, setMapPhotos] = useState<{ url: string; label: string }[]>([]);
  ...
  // Calculate scaling so the A4 doc fits in the center column
  const isMobileOrConstrained = containerWidth < DOCUMENT_WIDTH;
  const availableWidth = containerWidth - 48; // 24px padding on each side
  const baseScale = availableWidth / DOCUMENT_WIDTH;
  const effectiveScale = isMobileOrConstrained
    ? (zoom / 100) * baseScale
    : zoom / 100;

  const actualDocHeight = docRenderedHeight > 0 ? docRenderedHeight : DOCUMENT_WIDTH * 1.414;
  const heightCompensation = isMobileOrConstrained
    ? actualDocHeight * (1 - effectiveScale)
    : 0;
  ...
  return (
    // STRICT H-SCREEN LAYOUT: Prevents the entire page from scrolling
    <div className="flex flex-col h-screen bg-gray-50 font-sans overflow-hidden">
      ...
      <div className="flex flex-1 overflow-hidden min-h-0">
        ...
        {/* COLUMN 1: LEFT SIDEBAR (Inputs & Summary) */}
        <div className="hidden md:flex w-[320px] shrink-0 flex-col bg-white border-r border-gray-200 z-10">
          ...
          <div className="shrink-0 bg-white border-t border-gray-200 p-5 shadow-[0_-4px_6px_-4px_rgba(0,0,0,0.05)]">
            <h3 className="font-bold text-gray-900 mb-4 text-sm">Summary</h3>
            <div className="space-y-4">
              <div>
                <p className="text-gray-400 text-[10px] font-bold mb-0.5 uppercase tracking-wider">Type</p>
        ...
        {/* COLUMN 2: CENTER DOCUMENT PREVIEW (A4 Centered) */}
        <div className="flex-1 flex flex-col min-w-0 bg-[#e4f3fc] relative">
          ...
          <div ref={previewContainerRef} className="flex-1 overflow-y-auto overflow-x-hidden w-full py-8 flex flex-col items-center">
            <div ref={documentRef} className="print-safe-area flex flex-col items-center w-full">
              <div
                ref={docInnerRef}
                className="flex flex-col transform-container bg-white shadow-xl ring-1 ring-gray-900/5"
                style={{
                  width: `${DOCUMENT_WIDTH}px`,
                  minHeight: `${DOCUMENT_WIDTH * 1.414}px`, // A4 aspect ratio approximation
                  transform: `scale(${effectiveScale})`,
                  transformOrigin: 'top center',
                  marginBottom: `-${heightCompensation}px`,
                }}
        ...
        {/* COLUMN 3: RIGHT REPORT SECTION (Files & Actions) */}
        <div className="hidden lg:flex w-[350px] xl:w-[400px] shrink-0 flex-col bg-white border-l border-gray-200 shadow-[-4px_0_15px_-3px_rgba(0,0,0,0.05)] z-10">
          ...
          <div className="flex-1 overflow-y-auto p-5 xl:p-6 space-y-8">
            ...
            <div>
              <h3 className="font-medium text-gray-900 mb-4 text-sm flex items-center justify-between">
                Report Files
                <span className="bg-gray-100 text-gray-500 text-[10px] px-2 py-0.5 rounded-full">3 Files</span>
              </h3>
              <div className="space-y-3">
                {[
                  { name: 'Land Document', size: '4.2 MB' },
                  ...
                  <div key={idx} className="flex items-center justify-between p-3 border border-gray-200 rounded-xl bg-gray-50/50 hover:bg-gray-50 hover:border-blue-200 transition-colors group cursor-pointer">
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        <p className="text-[13px] font-semibold text-gray-800">{file.name}</p>
                        <p className="text-[11px] text-gray-500 font-medium mt-0.5">{file.size}</p>
                      </div>
                    </div>
                    <button className="p-2 text-gray-400 group-hover:text-blue-600 transition-colors bg-white rounded-md border border-gray-100 shadow-sm">
                      <Download size={16} />
                    </button>
                  </div>
                ))}
  ...
  <div className="grid grid-cols-1 gap-3 mb-5">
    {/* Accept Radio */}
    <label className="flex items-start gap-3 p-4 border-2 border-[#1B65D5] rounded-xl cursor-pointer bg-blue-50/10 transition-colors">
      <input 
        type="radio" 
        name="decision" 
        value="accept"
      ...
      />
      <div>
        <p className="text-[11px] font-medium text-blue-600 uppercase tracking-wide mb-0.5">Accept</p>
        <p className="font-medium text-gray-900 text-[13px]">Direct Approval</p>
      </div>
    </label>

    {/* On Hold Radio */}
    <label className="flex items-start gap-3 p-4 border border-gray-200 rounded-xl cursor-pointer hover:bg-gray-50 hover:border-gray-300 transition-colors">
      <input 
        type="radio" 
        name="decision" 
        value="hold"
      ...
      />
      <div>
        <p className="text-[11px] font-medium text-gray-500 uppercase tracking-wide mb-0.5">On Hold</p>
        <p className="font-medium text-gray-900 text-[13px]">Hold Report</p>
      </div>
    </label>
  </div>
  ...
  <div className="mb-5">
    ...
    <textarea 
      rows={4} 
      placeholder="Add notes for other editors..."
      className="w-full p-4 border border-gray-200 rounded-xl text-[13px] font-medium focus:outline-none focus:ring-2 focus:ring-[#1B65D5]/30 focus:border-[#1B65D5] resize-none text-gray-700 bg-gray-50/50"
    ></textarea>
  </div>
  ...
  <button className="w-full py-3.5 bg-[#1B65D5] text-white text-[13px] font-medium tracking-wide rounded-xl hover:bg-blue-700 transition-all shadow-[0_4px_14px_rgba(27,101,213,0.3)] hover:shadow-[0_6px_20px_rgba(27,101,213,0.4)]">
    Send Decision
  </button>
</div>
      ...
      <style dangerouslySetInnerHTML={{
        __html: `
          @media print {
            @page { margin: 0; size: A4 portrait; }
            body { -webkit-print-color-adjust: exact; print-color-adjust: exact; background: white !important; }
            .transform-container { transform: none !important; width: 100% !important; margin: 0 !important; gap: 0 !important; min-height: auto !important; box-shadow: none !important; }
            .a4-page { width: 210mm !important; height: 297mm !important; margin: 0 !important; padding: 15mm 20mm !important; box-shadow: none !important; page-break-after: always !important; }
            .no-print { display: none !important; }
          }
        `
