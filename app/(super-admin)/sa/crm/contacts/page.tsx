...
import { Search, Plus, Filter, Download, Users, Briefcase, Star, Flame, Link as LinkIcon } from 'lucide-react';
...
export default function ContactsPage() {
  ...
  const topTabs = [
    ...
    { name: 'Connected Contacts', icon: LinkIcon }, // <-- New Tab Added
  ];
    ...
    switch (activeTab) {
      ...
      case 'Customers':
        ...
        case 'Connected Contacts':
          // Bulletproof check: Ensure it exists and isn't an empty string
          return contacts.filter(c => c.linkedWith && c.linkedWith.trim() !== '');
      case 'All':
      default:
        return contacts;
    }
  ...
  return (
    <div className="flex flex-col h-full bg-white px-4 md:px-8 py-6">
        ...
        {topTabs.map((tab) => (
          <button
            ...
            className={`flex items-center gap-2 pb-2 text-sm font-medium whitespace-nowrap transition-colors relative
              ${activeTab === tab.name ? 'text-[#1B65D5]' : 'text-gray-500 hover:text-gray-800'}`}
          >
      ...
      <div className="flex items-center justify-between mb-4">
        ...
        {/* Action Buttons Typography Updated */}
      ...
      {/* Passed activeTab to ContactsList */}
      {viewType === 'list' ? (
        <ContactsList 
      ...
      <CreateContactModal 
        ...
        // onOpenCompany={() => {}} // Assuming handled inside or passed properly
      />
    </div>
  );
}